...
class Login extends React.Component {
    ...
    login = (e) => {
        ...
        e.preventDefault();
        e.stopPropagation();
        dispatch({type: Constants.LOGIN_REQUESTED, password: this.refs['password'].value, accountName: this.refs['accountName'].value});
    }
    render() {
        return (
            <div className={style['login__bg']} >
                <div className={style['login__box']} >
                    ...
                    <p className={classnames(t_style['text-center'], style['login--title'])} >商品中心管理平台</p>
                    <p className={classnames(t_style['text-center'], c_style['mb0'])} >
                        <img src="images/easy_start.png"/>
                    </p>
                    <div className={style['login__form']} >
                        <form>
                            <input className={classnames(style['login__input'], style['login--usericon'])} placeholder='用户名' type="text" ref="accountName"/>
